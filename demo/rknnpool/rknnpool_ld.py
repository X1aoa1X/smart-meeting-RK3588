...
def initRKNN(rknnModel="./rknnModel/best.rknn", id=-1):
    ...
    if ret != 0:
        ...
        raise RuntimeError(f"Load RKNN model failed with code {ret}")
    if id == 0:
    ...
    if ret != 0:
        ...
        raise RuntimeError(f"Init runtime environment failed with code {ret}")
...
class rknnPoolExecutor():
    def __init__(self, rknnModel, TPEs, func):
        ...
        self.func = func
        self.num = 0
        self._dev_lock = threading.Lock()  # 保护 rknnPool[0] 的线程安全访问
    ...
    def get(self):
        ...
        try:
            ...
            print(f"[rknnPool] inference worker failed: {e}")
            return None, False
    ...
    def release(self):
        ...
        for rknn_lite in self.rknnPool:
            try:
                ...
                print(f"[rknnPool] release failed: {e}")
